...
5. Соберите тестовый проект `cpp/practice01`:
    * Windows:
        ...
        5. Скопируйте необходимые библиотеки (`.../lib/x64/SDL3.dll` и `.../lib/wgpu_native.dll`) в директорию с собранным проектом (т.е. рядом с исполняемым файлом `practice01`) и погорюйте о том, что в Windows нет концепции [RPath](https://en.wikipedia.org/wiki/Rpath)
